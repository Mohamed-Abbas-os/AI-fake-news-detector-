...
function demoAnalyze(text, url) {
  ...
  const verdict = isFictional || hasUrgency || hasAbsolute ? 'NEEDS_VERIFICATION' : 'NEEDS_VERIFICATION';
  const signals = [];
  ...
  return { verdict, confidence: isFictional ? 78 : 61, riskLevel: isFictional || hasUrgency ? 'MEDIUM' : 'LOW', claims: [claim], riskSignals: signals, sourceAnalysis: url ? `The supplied source (${new URL(normaliseUrl(url)).hostname}) was noted, but this demo cannot verify its publication history or supporting evidence.` : 'No source was supplied. Attribution and publication context could not be assessed.', evidenceSummary: 'Insufficient evidence for verification. This frontend demo does not access live reporting, databases, or primary documents.', explanation: 'The claim contains signals that warrant a slower read, but language patterns alone do not prove a story is false. Compare the claim with reliable, independently reported evidence before sharing.', verificationSteps: ['Search for the same claim in two reputable, independent outlets.', 'Look for an original statement, official document, or named expert.', 'Check the date, location, entities, and whether the claim has been updated.'] };
}
